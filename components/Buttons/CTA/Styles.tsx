...
const dimmedCtaCss = css<StyledCtaProps>`
  background: none;
  background-color: ${({ $lightText, theme }) =>
    convertHexToRGBA($lightText ? theme.color__white : theme.color__gray, 15)};
  box-shadow: none;
  color: ${({ $lightText, theme }) => ($lightText ? theme.color__white : theme.color__gray)};
...
export const StyledCTA = styled.a<StyledCtaProps>`
  display: inline-flex;
  flex-direction: row;
  padding: 1em 2em;
